...
line1+="${sep}"
line1+="${dim}ctx${reset} ${pct_color}${pct_used}%${reset}"
line1+="${sep}"
line1+="${dim}dir${reset} ${cyan}${dirname}${reset}"
if [ -n "$git_branch" ]; then
    line1+=" ${green}(${git_branch}${red}${git_dirty}${green})${reset}"
fi
if [ -n "$session_duration" ]; then
    line1+="${sep}"
    line1+="${dim}elapsed${reset} ${white}${session_duration}${reset}"
fi
...
line1+="${sep}"
if $thinking_on; then
    line1+="${magenta}thinking on${reset}"
else
    line1+="${dim}thinking off${reset}"
fi
...
if [ -n "$usage_data" ] && echo "$usage_data" | jq -e . >/dev/null 2>&1; then
    ...
    if [ "$extra_enabled" = "true" ]; then
        ...
        extra_reset=$(/bin/date -v+1m -v1d +"%b %-d" 2>/dev/null | tr '[:upper:]' '[:lower:]')
